...
export const Badge = ({children, color = BADGE_COLORS.GRAY, ...rest}: PropsWithChildren<BadgeProps>) => {
    const badgeClasses = `inline-flex px-2 py-1 items-center font-medium rounded-full w-fit`;

    return (
        <span className={`${badgeClasses} ${badgeVariant[color]} ${badgeTextColorVariant[color]}`} {...rest}>
      {children}
        </span>
    );
